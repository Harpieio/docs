...
function Footer() {
  ...
  return (
    <footer className="text-white bg-gradient-to-l to-harple-700 via-harple-600 via-90 from-harpie">
      <div className="bg-neutral-900 py-12 rounded-b-3xl max-md:shadow-[4px_4px_46.29999923706055px_0px_#00000040]">
        <div className="relative flex flex-col w-full h-full gap-10 px-8 mx-auto overflow-hidden lg:justify-between lg:flex-row md:flex-col">
          <Link href={logo.href} className="block lg:hidden">
              <img src={logo.src} alt={logo.alt} className="mb-1 mr-2 w-28"/>
          </Link>
          <Link className="hidden lg:items-start lg:block" href={logo.href}>
              <img src={logo.src} alt={logo.alt} className="mb-1 mr-2 w-28"/>
          </Link>
          <div className="flex flex-col justify-between font-sans text-sm md:text-base lg:items-end">
            <div className="flex flex-col gap-8 lg:gap-4 xl:gap-8 lg:flex-row lg:justify-end">
              {links[0].items.map((link, index) => (
                <Link
                  key={index}
                  href={link.href}
                  className="text-white transition duration-200 ease-in-out hover:text-harpie-200"
                >
                  {link.label}
                </Link>
              ))}
